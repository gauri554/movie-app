...
export default function PaymentPage() {
  ...
  const confirmBooking = () => {
    setShowPopup(false);
    router.push(
      `/event-paymentsuccess?seats=${encodeURIComponent(
        seats.join(",")
      )}&price=${pricePerSeat}`
    );
  };
        ...
        return (
          <div className="mt-3 space-y-2">
            ...
            <select className="w-full px-2 py-1 sm:px-3 sm:py-2 rounded-lg bg-white/10 border border-white/20 text-xs sm:text-base">
              <option className="text-xs sm:text-base bg-white/10">HDFC Bank</option>
              <option className="text-xs sm:text-base bg-white/10">ICICI Bank</option>
              <option className="text-xs sm:text-base bg-white/10">State Bank of India</option>
              <option className="text-xs sm:text-base bg-white/10">Axis Bank</option>
            </select>
          </div>
        );
      case "wallet":
        return (
          <div className="mt-3 space-y-2">
            ...
            <select className="w-full px-3 py-2 rounded-lg bg-white/10 border border-white/20 text-xs sm:text-base">
              <option className="text-xs sm:text-base bg-white/10">Paytm</option>
              <option className="text-xs sm:text-base bg-white/10">PhonePe Wallet</option>
              <option className="text-xs sm:text-base bg-white/10">Amazon Pay</option>
            </select>
          </div>
        );
      default:
    ...
    <div className="min-h-screen bg-gradient-to-b from-[#0a1930] via-[#112d4e] to-[#0a1930] text-white  py-2 font-inter">
      ...
      <div className="grid lg:grid-cols-3 gap-6 max-w-7xl mx-auto p-6">
        ...
        <aside className="lg:col-span-1 bg-white/5 rounded-xl p-6 shadow-lg">
          <h2 className="text-sm text-center sm:text-left sm:text-lg md:text-xl font-bold mb-6">Booking Summary</h2>
        ...
        <main className="lg:col-span-2 bg-white/5 rounded-xl p-6 shadow-lg space-y-6">
          <h2 className="text-sm text-center sm:text-left sm:text-lg md:text-xl font-bold">Choose Payment Method</h2>
          ...
          >
            ...
            >
              ...
              <p className="text-gray-300 mb-6 text-[10px] sm:text-sm md:text-base">
                You are about to pay <span className="font-semibold">₹{total}</span> 
                for <span className="font-semibold">{seats.length}</span> seat(s) 
                using <span className="font-semibold">{selectedMethod?.toUpperCase()}</span>.
              </p>
